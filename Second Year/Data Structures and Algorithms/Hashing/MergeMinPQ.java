...
public class MergeMinPQ {
    ...
    public static int[] merge(MinPQ<Integer> one, MinPQ<Integer> two) {
        ...
        while(i < result.length && !one.isEmpty() && !two.isEmpty()){
            if (one.min() <= two.min()){
                result[i] = one.min();
                one.delMin();
                i++;
        ...
        if(!one.isEmpty()){
            while (i < result.length && !one.isEmpty()){
                result[i]= one.delMin();
                i++;
            }
        }
        if(!two.isEmpty()){
            while (i < result.length && !two.isEmpty()){
                result[i]= two.delMin();
                i++;
            }
        }

        // This checks that the array is sorted before returning!
