...
public class GreenHouseNursery extends AbsGreenHouse implements Sensible, QualityControlable{
    ...
    public void pollSensorData(List<Double> values) {
        ...
        double dudeGuy = clockAsDatetime();
        for (int i = 0; i < values.size(); i++){
           if ((values.get(i) > 1000) && (values.get(i) >= dudeGuy)){
               temp.add(values.get(i));
               dudeGuy = values.get(i);
               i++;
    ...
    public TempHumidReading middleReading() {
        ...
        return new SuperTempHumidReading((midReadingHelper(stored,1)), (midReadingHelper(stored, 2)));
    }
    ...
    public TempHumidReading middleReading(double onDate) {
       return super.midOnDate(stored, onDate);
    }
    ...
    public double percentError() {
        List<Double> dateless = removeDates(stored);
        int amount = 0;
