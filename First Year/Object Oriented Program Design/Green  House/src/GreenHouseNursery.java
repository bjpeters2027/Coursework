...
public class GreenHouseNursery extends AbsGreenHouse implements Sensible{
    ...
    public TempHumidReading middleReading(double onDate) {
        ...
        int i = 0;
        ArrayList<Double> data = new ArrayList<>(NoTimes(stored));
        List<Double> humids = new ArrayList<Double>();
        List<Double> temps = new ArrayList<Double>();
        if(!(data.contains(onDate))){
            return new SuperTempHumidReading(-999.0, -999.0);
        }
        for(int j = 0; j < data.size(); j++){
            if (data.get(j) == onDate){
                i = j;
            }
        }
        ...
        int k = i + 1;
        while((k < data.size()) && (data.get(k) < 1000)){
            dateList.add(data.get(k));
            k++;
        }
